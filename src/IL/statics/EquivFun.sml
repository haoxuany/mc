...
functor EquivFun(
  ...
  fun sgValid ctx sg =
    case sg of
      Sg_unit => ()
    | Sg_kind k => kindValid ctx k
    | Sg_type t => kindCheck ctx t Kind_type
    | Sg_lam (s, s') =>
        (sgValid ctx s;
  ...
  fun sgEquiv ctx sg sg' =
    case (sg, sg') of
      (Sg_unit, Sg_unit) => ()
    | (Sg_kind k, Sg_kind k') => kindEquiv ctx k k'
  ...
  fun subsg ctx sg sg' =
    case (sg, sg') of
      (Sg_unit, Sg_unit) => ()
    (* no need to consider k === k', because subKinding is already
    * admissible for that judgement *)
    | (Sg_kind k, Sg_kind k') => subkind ctx k k'
    (* This is admissible from s <= s' given s === s' *)
    | (Sg_type t, Sg_type t') => conEquiv ctx t t' Kind_type
    | (Sg_lam (s1, s1'), Sg_lam (s2, s2')) =>
        (subsg ctx s2 s1;
